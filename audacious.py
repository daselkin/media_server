...
class AudaciousController:
	def _execute_audacious(self, flags: list[str], file_name: str = '') -> Popen:
	...
	def load_file(self, file_name: str, enqueue: bool = False) -> Popen:
		...
		return self._execute_audacious(flags = ['--enqueue'] if enqueue else [], file_name)
	...
	def skip_forward(self) -> Popen:
		...
		return self._execute_audacious(flags=['--fwd'], file_name)

	def skip_back(self) -> Popen:
		logging.info("Skipping back")
		return self._execute_audacious(flags=['--rew'], file_name)
